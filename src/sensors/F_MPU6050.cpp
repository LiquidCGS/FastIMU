...
int MPU6050::init(calData cal, uint8_t address) 
{
	...
	if (!(readByteI2C(wire, IMUAddress, MPU6050_WHO_AM_I_MPU6050) == MPU6050_WHOAMI_DEFAULT_VALUE)) {
		return -1;
	}

	// reset device
	writeByteI2C(wire, IMUAddress, MPU6050_PWR_MGMT_1, 0x80); // Write a one to bit 7 reset bit; toggle reset device
	delay(100);
	// wake up device
	writeByteI2C(wire, IMUAddress, MPU6050_PWR_MGMT_1, 0x00); // Clear sleep mode bit (6), enable all sensors
	...
	writeByteI2C(wire, IMUAddress, MPU6050_INT_PIN_CFG, 0x22);
	writeByteI2C(wire, IMUAddress, MPU6050_INT_ENABLE, 0x01);    // Enable data ready (bit 0) interrupt
	delay(100);
	return 0;
}
...
void MPU6050::update() {
	...
	readBytesI2C(wire, IMUAddress, MPU6050_ACCEL_XOUT_H, 14, &rawData[0]);    // Read the 14 raw data registers into data array
...
void MPU6050::calibrateAccelGyro(calData* cal) 
{
	...
	writeByteI2C(wire, IMUAddress, MPU6050_PWR_MGMT_1, 0x03);
	writeByteI2C(wire, IMUAddress, MPU6050_PWR_MGMT_2, 0x00);
	delay(200);

	// Configure device for bias calculation
	writeByteI2C(wire, IMUAddress, MPU6050_INT_ENABLE, 0x00);   // Disable all interrupts
	writeByteI2C(wire, IMUAddress, MPU6050_FIFO_EN, 0x00);      // Disable FIFO
	writeByteI2C(wire, IMUAddress, MPU6050_PWR_MGMT_1, 0x00);   // Turn on internal clock source
	writeByteI2C(wire, IMUAddress, MPU6050_I2C_MST_CTRL, 0x00); // Disable I2C master
	writeByteI2C(wire, IMUAddress, MPU6050_USER_CTRL, 0x00);    // Disable FIFO and I2C master modes
	writeByteI2C(wire, IMUAddress, MPU6050_USER_CTRL, 0x04);    // Reset FIFO
	delay(15);
	...
	writeByteI2C(wire, IMUAddress, MPU6050_USER_CTRL, 0x40);   // Enable FIFO
	writeByteI2C(wire, IMUAddress, MPU6050_FIFO_EN, 0x78);     // Enable gyro and accelerometer sensors for FIFO  (max size 512 bytes in MPU-9150)
	delay(40); // accumulate 40 samples in 40 milliseconds = 480 bytes

	// At end of sample accumulation, turn off FIFO sensor read
	writeByteI2C(wire, IMUAddress, MPU6050_FIFO_EN, 0x00);        // Disable gyro and accelerometer sensors for FIFO
	readBytesI2C(wire, IMUAddress, MPU6050_FIFO_COUNTH, 2, &data[0]); // read FIFO sample count
	...
	for (ii = 0; ii < packet_count; ii++)
	{
		...
		readBytesI2C(wire, IMUAddress, MPU6050_FIFO_R_W, 12, &data[0]); // read data for averaging
...
int MPU6050::setGyroODR(int odr_hz) {
	if (odr_hz <= 0) return -1;
	uint8_t div = (uint8_t)constrain(1000 / odr_hz - 1, 0, 255);
	writeByteI2C(wire, IMUAddress, MPU6050_SMPLRT_DIV, div);
