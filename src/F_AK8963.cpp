...
int AK8963::init(calData cal, uint8_t address) 
{
	...
	if (!(readByteI2C(wire, AK8963_ADDRESS, AK8963_WHO_AM_I) == AK8963_WHOAMI_DEFAULT_VALUE)) {
		return -1;
	}
...
void AK8963::getAccel(AccelData* out) 
{
	return;
}
void AK8963::getGyro(GyroData* out) 
{
	return;
}
void AK8963::getMag(MagData* out) 
{
	memcpy(out, &mag, sizeof(mag));
}

int AK8963::setAccelRange(int range) {
	return -1;
}

int AK8963::setGyroRange(int range) {
	return -1;
}

void AK8963::calibrateAccelGyro(calData* cal) 
{
	return;
}

void AK8963::calibrateMag(calData* cal) 
{
