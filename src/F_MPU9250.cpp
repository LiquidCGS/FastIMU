...
int MPU9250::init(calData cal, uint8_t address) 
{
	...
	writeByte(IMUAddress, MPU9250_MPU_CONFIG, 0x01);

	// Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
	writeByte(IMUAddress, MPU9250_SMPLRT_DIV, 0x00);  // Use a 1000 Hz rate; a rate consistent with the filter update rate
	...
	c = readByte(address, MPU9250_ACCEL_CONFIG); // get current ACCEL_CONFIG register value
	// c = c & ~0xE0; // Clear self-test bits [7:5]
...
void MPU9250::calibrateAccelGyro(float* out_accelBias, float* out_gyroBias) 
{
...
void MPU9250::calibrateMag(float* out_magBias, float* out_magScale) 
{
