...
int MPU9250::setAccelRange(uint8_t range) {
	uint8_t c;
	if (range >= 3) {
...
int MPU9250::setGyroRange(uint8_t range) {
	uint8_t c;
	if (range >= 3) {
	...
	c = readByte(IMUAddress, MPU9250_GYRO_CONFIG); // get current GYRO_CONFIG register value
	c = c & ~0x18;  // Clear AFS bits [4:3]
	c = c | range << 3; // Set full scale range for the accelerometer (11 on 4:3)
