...
void HMC5883L::update()
{
	...
	mz = ((float)(magCount[1] * mRes - calibration.magBias[1]) * calibration.magScale[1]) * 100.f;  // get actual magnetometer value, this depends on scale being set
	my = ((float)(magCount[2] * mRes - calibration.magBias[2]) * calibration.magScale[2]) * 100.f;  //mul by 100 to convert from G to µT
...
void HMC5883L::calibrateMag(calData* cal) 
{
	...
	sample_count = 1000;  // at 50 Hz ODR, new mag data is available every 20 ms
	...
	for (ii = 0; ii < sample_count; ii++)
	{
		...
		delay(21); // at 50 Hz ODR, new mag data is available every 20 ms
	}
